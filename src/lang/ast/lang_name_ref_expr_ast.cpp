...
lang_name_ref_expr::lang_name_ref_expr(code_location start, code_location end, std::string name)
    : base_ast(start, end, LANG_NAME_REF), name(std::move(name))
{
}
...
auto lang_name_ref_expr::serialize() const -> std::string { return fmt::format("(language_name_reference \"{}\")", name); }
